    ...
    for (const auto & elem : this->_mesh->active_local_element_ptr_range())
      {
        ...
        // 1D Elements don't have interior angles, so the base class
        // implementation currently returns 0 for those elements.
        // Reference triangles/tetrahedra have min interior angle of 45 degrees.
        if (elem->dim() > 1)
          CPPUNIT_ASSERT_GREATEREQUAL(Real(45 - TOLERANCE), min_angle);
      }
  }
